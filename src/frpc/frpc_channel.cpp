// FRPC Channel implementation — client-side HTTP/2 transport with FlatBuffers
...
Task<bool> channel::connect() {
    ...
    if (err != 0) {
        std::cerr << "[frpc] Connect failed with error: " << err << std::endl;
        co_return false;
    }
    ...
    while (preface_sent < 24) {
        ...
        if (n <= 0) {
            std::cerr << "[frpc] Failed to send H2 preface" << std::endl;
            sock_.close();
            co_return false;
        }
...
Task<bool> channel::connect(ssl::context& ssl_ctx) {
    std::cerr << "[frpc] TLS connect not yet implemented" << std::endl;
    co_return false;
}
#endif
...
// ============================================================================
// Helper: build HTTP/2 FRPC request
// ============================================================================

static http::request make_frpc_request(const std::string& host,
    ...
    req.hdrs.set("Content-Type", frpc_constants::content_type);
    req.hdrs.set("TE", frpc_constants::te);
    req.hdrs.set("grpc-encoding", frpc_constants::grpc_encoding);
    if (!body_data.empty()) {
...
// ============================================================================
// Helper: extract FRPC status from response
// ============================================================================

static status extract_frpc_status(const http::response& resp) {
    status rpc_status{status_code::ok, ""};

    auto status_hdr = resp.hdrs.get(frpc_constants::grpc_status);
    if (status_hdr) {
        int code = 0;
    ...
    auto msg_hdr = resp.hdrs.get(frpc_constants::grpc_message);
    if (msg_hdr) {
        rpc_status.message = *msg_hdr;
    }
    ...
    const std::string& request_data)
{
    ...
    auto req = make_frpc_request(host_, path, encode_grpc_message(request_data));
    ...
    status rpc_status = extract_frpc_status(resp);
    if (!rpc_status.ok()) {
        co_return std::make_pair(rpc_status, std::string{});
    }

    auto response_data = decode_grpc_message(resp.bd.data());
    if (!response_data) {
        co_return std::make_pair(
            status{status_code::internal, "Invalid FRPC response frame"},
            std::string{});
    }

    co_return std::make_pair(rpc_status, std::move(*response_data));
}
    ...
    std::function<void(const std::string&)> on_message)
{
    ...
    co_return extract_frpc_status(promise->resp);
}
    ...
    const std::string& method)
{
    ...
    std::string path = make_path(service, method);
    auto req = make_frpc_request(host_, path, "");
