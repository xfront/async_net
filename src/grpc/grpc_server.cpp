// gRPC Server implementation — built on HTTP/2 transport
...
void server::register_method(const std::string& service, const std::string& method, method_handler handler) {
    ...
    std::cout << "[grpc] Registered method (unary): " << path << std::endl;
}
...
void server::register_method(const std::string& service, const std::string& method, method_handler_with_context handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (unary+ctx): " << path << std::endl;
}
...
void server::register_server_stream(const std::string& service, const std::string& method, server_stream_handler handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (server-stream): " << path << std::endl;
}
...
void server::register_server_stream(const std::string& service, const std::string& method, server_stream_handler_with_context handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (server-stream+ctx): " << path << std::endl;
}
...
void server::register_client_stream(const std::string& service, const std::string& method, client_stream_handler handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (client-stream): " << path << std::endl;
}
...
void server::register_client_stream(const std::string& service, const std::string& method, client_stream_handler_with_context handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (client-stream+ctx): " << path << std::endl;
}
...
void server::register_bidi_stream(const std::string& service, const std::string& method, bidi_stream_handler handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (bidi-stream): " << path << std::endl;
}
...
void server::register_bidi_stream(const std::string& service, const std::string& method, bidi_stream_handler_with_context handler) {
    ...
    methods_[path] = std::move(e);
    std::cout << "[grpc] Registered method (bidi-stream+ctx): " << path << std::endl;
}
...
void server::add_interceptor(interceptor_fn interceptor) {
...
Task<std::pair<status, std::string>> server::dispatch(const std::string& path, const std::string& protobuf_data) {
    call_context ctx;
    return dispatch(path, protobuf_data, ctx);
}

Task<std::pair<status, std::string>> server::dispatch(const std::string& path, const std::string& protobuf_data, call_context& ctx) {
    auto it = methods_.find(path);
    if (it == methods_.end()) {
        std::cerr << "[grpc] Method not found: " << path << std::endl;
    ...
    if (!interceptors_.empty()) {
        ...
        call_info info(svc, mtd, ctx.initial_metadata);
        std::string error_response;
        status error_status;
        auto chain_task = interceptors_.run(info, protobuf_data, error_response, error_status);
        chain_task.resume();
        if (chain_task.done()) {
    ...
    try {
        Task<std::string> task = it->second.has_context
            ? it->second.unary_ctx(protobuf_data, ctx)
            : it->second.unary(protobuf_data);
        task.resume();
        if (task.done()) {
        ...
        std::cerr << "[grpc] Handler exception: " << e.what() << std::endl;
        co_return std::make_pair(status{status_code::internal, e.what()}, std::string{});
    }
}

// ============================================================================
// Handle a plain HTTP/2 connection (gRPC with prior knowledge, no TLS)
// ============================================================================
...
Task<void> server::handle_connection(tcp::socket sock) {
    ...
    session.set_request_handler([this](const request& req) -> response {
        ...
        if (!ct || ct->find("application/grpc") != 0) {
            ...
            resp.bd = http::body("Not a gRPC request");
            return resp;
        }
        ...
        if (req.method != http::method::POST) {
            ...
            resp.bd = http::body("gRPC requires POST method");
            return resp;
        }

        auto protobuf_data = decode_grpc_message(req.bd.data());
        if (!protobuf_data) {
            response resp;
            resp.status = http::status_code::ok();
            resp.hdrs.set("content-type", grpc_constants::content_type);
            resp.trailers.set(grpc_constants::grpc_status, std::to_string(static_cast<int>(status_code::internal)));
            resp.trailers.set(grpc_constants::grpc_message, "Invalid gRPC frame");
            return resp;
        }
        ...
        auto task = dispatch(req.path, *protobuf_data, ctx);
        task.resume();
        
        status grpc_status{status_code::internal, "Handler did not complete"};
        ...
        resp.status = http::status_code::ok();
        resp.hdrs.set("content-type", grpc_constants::content_type);
        resp.hdrs.set("grpc-encoding", grpc_constants::grpc_encoding);

        if (!response_data.empty()) {
            resp.bd = http::body(encode_grpc_message(response_data));
        }

        resp.trailers.set(grpc_constants::grpc_status, std::to_string(static_cast<int>(grpc_status.code)));
        if (!grpc_status.message.empty()) {
            resp.trailers.set(grpc_constants::grpc_message, grpc_status.message);
        }

        // Add response metadata as trailers
    ...
    // Request start handler — called when HEADERS arrive (before body)
    // Detects streaming methods and sets up data callbacks
    session.set_request_start_handler([this, &session](int32_t stream_id, const request& req, bool end_stream) -> bool {
        auto ct = req.hdrs.get("content-type");
        if (!ct || ct->find("application/grpc") != 0) {
            return false; // Not gRPC
        }

        auto path_it = methods_.find(req.path);
        if (path_it == methods_.end() || path_it->second.type == handler_type::unary) {
            return false; // Unknown or unary — let normal flow handle it
        }

        auto& entry = path_it->second;

        // Send response headers (no body, no END_STREAM)
        http::response resp;
        resp.status = http::status_code::ok();
        resp.hdrs.set("content-type", grpc_constants::content_type);
        resp.hdrs.set("grpc-encoding", grpc_constants::grpc_encoding);
        ...
        if (entry.type == handler_type::server_stream) {
            ...
            session.set_data_callback(stream_id,
                [buffer, wrt, &entry, handler_called, &session, stream_id](int32_t, const uint8_t* data, size_t len, bool es) {
                    ...
                    if (es && !*handler_called) {
                        *handler_called = true;
                        auto protobuf_data = decode_grpc_message(*buffer);
                        std::string request_data = protobuf_data.value_or("");
            ...
            // If END_STREAM was on HEADERS (no body), invoke handler immediately
        ...
        if (entry.type == handler_type::client_stream) {
            ...
            // If END_STREAM was on HEADERS (no body), launch immediately
            if (end_stream) {
                deframer->complete();
            ...
            // Bidi: accumulate messages, launch handler on END_STREAM
            auto handler_launched = std::make_shared<bool>(false);
            auto entry_ptr = &entry;
    ...
    // Read client connection preface (24 bytes)
    {
        ...
        if (preface_read < PREFACE_LEN ||
            ...
            std::cerr << "[grpc] Invalid client connection preface" << std::endl;
            sock.close();
            co_return;
        }
...
Task<void> server::serve() {
    std::cout << "[grpc] Server listening on port " << port_ << std::endl;

    while (running_) {
        auto sock = co_await acceptor_.async_accept();
        if (!sock.is_open()) {
            if (running_) {
                std::cerr << "[grpc] Accept failed" << std::endl;
            }
            continue;
        }
...
Task<void> server::handle_tls_connection(tcp::socket sock, ssl::context& ssl_ctx) {
    ...
    // Same handlers as plain connection
    session.set_request_handler([this](const request& req) -> response {
        auto ct = req.hdrs.get("content-type");
        if (!ct || ct->find("application/grpc") != 0) {
            response resp;
    ...
    // Request start handler (same logic as plain connection)
    session.set_request_start_handler([this, &session](int32_t stream_id, const request& req, bool end_stream) -> bool {
        auto ct = req.hdrs.get("content-type");
...
Task<void> server::serve_tls(ssl::context& ssl_ctx) {
    std::cout << "[grpc] Server listening on port " << port_ << " (TLS)" << std::endl;
